...
scanInitialStates == (((((((pc = "scanInitialStates") /\ (initialIndex <= Len(frontier))) /\ (TRUE /\ currentState' = frontier[initialIndex])) /\ ((closed' = (closed \cup {frontier[initialIndex]}) /\ levels' = [__typedPartialFunctionOverrideEntry \in (DOMAIN levels \cup {frontier[initialIndex]}) |-> (IF (__typedPartialFunctionOverrideEntry = frontier[initialIndex]) THEN 0 ELSE levels[__typedPartialFunctionOverrideEntry])]) /\ (initialIndex' = (initialIndex + 1) /\ (frontier[initialIndex] \in {4})))) /\ (((TRUE /\ counterexample' = <<frontier[initialIndex]>>) /\ (pc' = "trace" /\ UNCHANGED frontier)) /\ (UNCHANGED successors /\ UNCHANGED predecessorEdges))) \/ (((((pc = "scanInitialStates") /\ (initialIndex <= Len(frontier))) /\ (TRUE /\ currentState' = frontier[initialIndex])) /\ ((closed' = (closed \cup {frontier[initialIndex]}) /\ levels' = [__typedPartialFunctionOverrideEntry \in (DOMAIN levels \cup {frontier[initialIndex]}) |-> (IF (__typedPartialFunctionOverrideEntry = frontier[initialIndex]) THEN 0 ELSE levels[__typedPartialFunctionOverrideEntry])]) /\ (initialIndex' = (initialIndex + 1) /\ (~(frontier[initialIndex] \in {4}))))) /\ (((TRUE /\ pc' = "scanInitialStates") /\ (UNCHANGED frontier /\ UNCHANGED successors)) /\ (UNCHANGED counterexample /\ UNCHANGED predecessorEdges)))) \/ (((((pc = "scanInitialStates") /\ (~(initialIndex <= Len(frontier)))) /\ (pc' = "checkInitialStates" /\ UNCHANGED frontier)) /\ ((UNCHANGED closed /\ UNCHANGED currentState) /\ (UNCHANGED successors /\ UNCHANGED initialIndex))) /\ ((UNCHANGED counterexample /\ UNCHANGED predecessorEdges) /\ UNCHANGED levels)))
...
exploreSuccessors == (((((((pc = "exploreSuccessors") /\ TRUE) /\ ((Cardinality(successors) = 0) /\ TRUE)) /\ ((pc' = "dequeue" /\ UNCHANGED frontier) /\ (UNCHANGED closed /\ UNCHANGED currentState))) /\ (((UNCHANGED successors /\ UNCHANGED initialIndex) /\ (UNCHANGED counterexample /\ UNCHANGED predecessorEdges)) /\ UNCHANGED levels)) \/ ((((pc = "exploreSuccessors") /\ TRUE) /\ ((~(Cardinality(successors) = 0)) /\ TRUE)) /\ ((\E __binder_TLCMC_109_25 \in successors: ((((TRUE /\ successors' = (successors \ {__binder_TLCMC_109_25})) /\ (closed' = (closed \cup {__binder_TLCMC_109_25}) /\ frontier' = Append(frontier, __binder_TLCMC_109_25))) /\ ((predecessorEdges' = Append(predecessorEdges, <<(CASE currentState \in {1, 2, 3, 4} -> currentState), __binder_TLCMC_109_25>>) /\ levels' = [__typedPartialFunctionOverrideEntry \in (DOMAIN levels \cup {__binder_TLCMC_109_25}) |-> (IF (__typedPartialFunctionOverrideEntry = __binder_TLCMC_109_25) THEN (levels[(CASE currentState \in {1, 2, 3, 4} -> currentState)] + 1) ELSE levels[__typedPartialFunctionOverrideEntry])]) /\ ((__binder_TLCMC_109_25 \in {4}) /\ TRUE))) /\ (counterexample' = <<(CASE currentState \in {1, 2, 3, 4} -> currentState), __binder_TLCMC_109_25>> /\ pc' = "trace")) /\ UNCHANGED currentState) /\ UNCHANGED initialIndex))) \/ ((((pc = "exploreSuccessors") /\ TRUE) /\ ((~(Cardinality(successors) = 0)) /\ TRUE)) /\ ((\E __binder_TLCMC_109_25 \in successors: ((((TRUE /\ successors' = (successors \ {__binder_TLCMC_109_25})) /\ (closed' = (closed \cup {__binder_TLCMC_109_25}) /\ frontier' = Append(frontier, __binder_TLCMC_109_25))) /\ ((predecessorEdges' = Append(predecessorEdges, <<(CASE currentState \in {1, 2, 3, 4} -> currentState), __binder_TLCMC_109_25>>) /\ levels' = [__typedPartialFunctionOverrideEntry \in (DOMAIN levels \cup {__binder_TLCMC_109_25}) |-> (IF (__typedPartialFunctionOverrideEntry = __binder_TLCMC_109_25) THEN (levels[(CASE currentState \in {1, 2, 3, 4} -> currentState)] + 1) ELSE levels[__typedPartialFunctionOverrideEntry])]) /\ ((~(__binder_TLCMC_109_25 \in {4})) /\ TRUE))) /\ pc' = "exploreSuccessors") /\ UNCHANGED currentState) /\ (UNCHANGED initialIndex /\ UNCHANGED counterexample))))
trace == (((((((pc = "trace") /\ TRUE) /\ (TRUE /\ (Head(counterexample) \in {1, 2}))) /\ ((TRUE /\ TRUE) /\ (pc' = "Done" /\ UNCHANGED frontier))) /\ (((UNCHANGED closed /\ UNCHANGED currentState) /\ (UNCHANGED successors /\ UNCHANGED initialIndex)) /\ ((UNCHANGED counterexample /\ UNCHANGED predecessorEdges) /\ UNCHANGED levels))) \/ (((((pc = "trace") /\ TRUE) /\ (TRUE /\ (~(Head(counterexample) \in {1, 2})))) /\ ((TRUE /\ counterexample' = (<<SelectSeq(predecessorEdges, LAMBDA __binder_TLCMC_134_43: (__binder_TLCMC_134_43[2] = counterexample[1]))[1][1]>> \o counterexample)) /\ (pc' = "trace" /\ UNCHANGED frontier))) /\ (((UNCHANGED closed /\ UNCHANGED currentState) /\ (UNCHANGED successors /\ UNCHANGED initialIndex)) /\ (UNCHANGED predecessorEdges /\ UNCHANGED levels)))) \/ (((((pc = "trace") /\ (~TRUE)) /\ (pc' = "Done" /\ UNCHANGED frontier)) /\ ((UNCHANGED closed /\ UNCHANGED currentState) /\ (UNCHANGED successors /\ UNCHANGED initialIndex))) /\ ((UNCHANGED counterexample /\ UNCHANGED predecessorEdges) /\ UNCHANGED levels)))
