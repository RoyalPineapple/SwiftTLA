...
scanInitialStates == (((((((pc = "scanInitialStates") /\ (initialIndex <= Len(frontier))) /\ (TRUE /\ currentState' = frontier[initialIndex])) /\ ((closed' = (closed \cup {frontier[initialIndex]}) /\ levels' = [__binder_StateExpr_355_13 \in (DOMAIN levels \cup {frontier[initialIndex]}) |-> (IF (__binder_StateExpr_355_13 = frontier[initialIndex]) THEN 0 ELSE levels[__binder_StateExpr_355_13])]) /\ (initialIndex' = (initialIndex + 1) /\ (frontier[initialIndex] \in {4})))) /\ (((TRUE /\ counterexample' = <<frontier[initialIndex]>>) /\ (pc' = "trace" /\ UNCHANGED frontier)) /\ (UNCHANGED successors /\ UNCHANGED predecessorEdges))) \/ (((((pc = "scanInitialStates") /\ (initialIndex <= Len(frontier))) /\ (TRUE /\ currentState' = frontier[initialIndex])) /\ ((closed' = (closed \cup {frontier[initialIndex]}) /\ levels' = [__binder_StateExpr_355_13 \in (DOMAIN levels \cup {frontier[initialIndex]}) |-> (IF (__binder_StateExpr_355_13 = frontier[initialIndex]) THEN 0 ELSE levels[__binder_StateExpr_355_13])]) /\ (initialIndex' = (initialIndex + 1) /\ (~(frontier[initialIndex] \in {4}))))) /\ (((TRUE /\ pc' = "scanInitialStates") /\ (UNCHANGED frontier /\ UNCHANGED successors)) /\ (UNCHANGED counterexample /\ UNCHANGED predecessorEdges)))) \/ (((((pc = "scanInitialStates") /\ (~(initialIndex <= Len(frontier)))) /\ (pc' = "checkInitialStates" /\ UNCHANGED frontier)) /\ ((UNCHANGED closed /\ UNCHANGED currentState) /\ (UNCHANGED successors /\ UNCHANGED initialIndex))) /\ ((UNCHANGED counterexample /\ UNCHANGED predecessorEdges) /\ UNCHANGED levels)))
