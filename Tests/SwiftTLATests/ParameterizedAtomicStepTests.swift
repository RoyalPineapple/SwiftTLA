...
struct ParameterizedAtomicStepTests {
    ...
    func exploresConfiguredDomains() throws {
        ...
        for (index, scenario) in scenarios.enumerated() {
            ...
            #expect(exhaustive.graph.graph.states.count == [1, 1, 2][index])
            #expect(exhaustive.graph.graph.edges.count == [0, 1, 6][index])
            #expect(exhaustive.checks.properties.values.allSatisfy { $0 == .satisfied })
            #expect(run.native.checks.properties.values.allSatisfy { $0 == (index == 0 ? .unavailable : .satisfied) })
            #expect(run.native.graph == (index == 0 ? nil : exhaustive.graph))
    ...
    @Test("parameterized steps reject malformed domains, bindings, and scheduled placement", arguments: [
        ...
        "Do(Step.select, over: Set<Int>([0]), Set<Int>([1]), Set<Int>([2])) { first, second in Skip() }",
        "Do(Step.select, over: Set<Int>([0]), Set<Int>([1]), Set<Int>([2])) { first, second, first in Skip() }",
        "Do(Step.select, over: Set<Int>([0]), Set<Int>([1]), selected) { first, second, third in Skip() }",
