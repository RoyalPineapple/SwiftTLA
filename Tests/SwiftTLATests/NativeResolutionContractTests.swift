...
@Suite struct NativeResolutionContractTests {
    ...
    func storedReadRepresentations() throws {
        ...
        let checker = try NativeTypeInference(compilation: compilation, sourceTypes: .init(enums: ["Node": [.int(1), .int(2)]]))
        let node = try #require(compilation.layout.variables.first { $0.declaration.name == "node" })
        let nodes = try #require(compilation.layout.variables.first { $0.declaration.name == "nodes" })
        for expression in [
    ...
    func boundReadRepresentations() throws {
        let compilation = try TLASpec(name: "BoundRead", variables: [
            .init(name: "node", initialization: .value(.int(1)), generatedSwiftType: "Node", origin: .compiler)
    ...
    func sequenceSourceRepresentations() throws {
        ...
        for operation in operations {
            let compilation = try TLASpec(name: "SequenceOperands", variables: [
    ...
    func foldOperandRepresentations() throws {
        ...
        for (source, sourceType) in sources {
            ...
            let compilation = try TLASpec(name: "FoldOperands", variables: [
    ...
    func conversionsFollowExpressionUses() throws {
        ...
        let program = try NativeResolvedProgram(compilation: specification.compile(), sourceTypes: metadata)
    ...
    func polymorphicUsesHaveSeparateBodies() throws {
        ...
        let compilation = try TLASpec(name: "ResolvedCalls", variables: [
    ...
    func localFunctionsCaptureOnlyReferencedValues() throws {
        ...
        let compilation = try TLASpec(name: "LocalCaptures", variables: [], actions: [],
            invariants: [.init(name: "Comparison", body: .lessThan(first, second))],
            formalOperatorDefinitions: [operation]).compile()
        let outer = try #require(compilation.semantics.formalOperatorDefinitions.first)
    ...
    func growingArgumentShapesAreRejected() throws {
        ...
        let compilation = try compileSpecification(operation: operation, arguments: [.value(.int(1)), .value(.int(0))], boolean: true)
    ...
    func growingRecordFieldsAreRejected() throws {
        ...
        let compilation = try compileSpecification(operation: operation, arguments: [
    ...
    func selectedSequenceAnnotations() throws {
        ...
        let compilation = try TLASpec(name: "SelectedSequence", variables: [
    ...
    private func compileSpecification(operation: FormalOperatorDefinition, arguments: [FormalCallArgument], boolean: Bool) throws -> CompiledSpecification {
        ...
        return try TLASpec(name: "ResolvedRecursion", variables: [
            .init(name: "number", initialization: .expression(.int(0)), origin: .compiler)
