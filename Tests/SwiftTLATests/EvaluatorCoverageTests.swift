...
struct EvaluatorCoverage {
    ...
    func quantifiers() throws {
        #expect(try StateExpr.forAll(.variable("s"), .greaterThan(.variable("_x"), .int(0))).evaluate(in: state) == .bool(true))
        #expect(try StateExpr.exists(.variable("s"), .equal(.variable("_x"), .int(2))).evaluate(in: state) == .bool(true))
    }
