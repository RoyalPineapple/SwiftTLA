            ...
            SwiftTLA.Action("accepted") {
                result.becomes(Expr<Int>(StateExpr.letIn([
                    LocalOperator("OnlyZero", parameters: ["value"],
                        domain: StateExpr.integerRange(0, 0), body: 7)
                ], StateExpr.variable("OnlyZero").applying(0))))
            }
            SwiftTLA.Action("rejected") {
                result.becomes(Expr<Int>(StateExpr.letIn([
                    LocalOperator("OnlyZero", parameters: ["value"],
                        domain: StateExpr.integerRange(0, 0), body: 7)
                ], StateExpr.variable("OnlyZero").applying(1))))
            }
            SwiftTLA.Action("domainFailure") {
                result.becomes(Expr<Int>(StateExpr.letIn([
                    LocalOperator("Bounded", parameters: ["value"],
                        domain: StateExpr.integerRange(StateExpr.variable("result") / 0, 0), body: 7)
                ], StateExpr.variable("Bounded").applying(
                    StateExpr.choose(StateExpr.integerRange(1, 0), "item", true)
                ))))
            }
            SwiftTLA.Action("argumentFailure") {
                result.becomes(Expr<Int>(StateExpr.letIn([
                    LocalOperator("OnlyZero", parameters: ["value"],
                        domain: StateExpr.integerRange(0, 0), body: 7)
                ], StateExpr.variable("OnlyZero").applying(
                    StateExpr.choose(StateExpr.integerRange(1, 0), "item", true)
                ))))
            }
            SwiftTLA.Action("bodyFailure") {
                result.becomes(Expr<Int>(StateExpr.letIn([
                    LocalOperator("OnlyZero", parameters: ["value"],
                        domain: StateExpr.integerRange(0, 0), body: StateExpr.variable("result") / 0)
                ], StateExpr.variable("OnlyZero").applying(0))))
            }
        }
    }
    ...
    static var spec: TLASpec {
        TLASpec("FunctionArgumentOrder") {
            ...
            FormalDefinition("Read", parameters: [.value("function"), .value("key")],
                body: StateExpr.variable("function").applying(StateExpr.variable("key")))
            SwiftTLA.Action("read") {
                result.becomes(FormalCall(as: Int.self, "Read",
                    If(Expr<Int>(1) / 0 == 0,
                        then: TupleExpr<Int>.literal(1),
                        else: TupleExpr<Int>.literal(2)),
                    Expr<Int>(9_223_372_036_854_775_807) + 1))
            }
        }
    }
...
@Suite struct NativeOperatorEvaluationTests {
    ...
    func boundedCallsPreserveFailureOrder() throws {
        ...
        var machine = try BoundedOperatorExecution.makeMachine()
        #expect(try machine.send(.accepted).after.result == 7)
        #expect(try successor.state.value(for: result) == .integer(machine.state.result))

        let failures: [(String, BoundedOperatorExecution.Action, EvalError, NativeMachineEvaluationError)] = [
            ("rejected", .rejected, .recursiveArgumentOutsideDomain, .functionArgumentOutsideDomain),
            ("domainFailure", .domainFailure, .divisionByZero, .divisionByZero),
            ("argumentFailure", .argumentFailure, .noSatisfyingChoice, .noSatisfyingChoice),
            ("bodyFailure", .bodyFailure, .divisionByZero, .divisionByZero)
        ]
        for (name, nativeAction, formalError, nativeError) in failures {
            let action = try #require(compilation.layout.testActionID(named: name))
            #expect(throws: formalError) {
                try runtime.successors(for: action, from: initial)
            }
            var machine = try BoundedOperatorExecution.makeMachine()
            let before = machine.state
            #expect(throws: nativeError) { try machine.send(nativeAction) }
            #expect(machine.state == before)
        }
    }
