...
struct VoteProofCorpusRenderingTests {
    ...
    func specMacroCompilationPreservesFormalStructure() throws {
        ...
        let fixtures = projectURL("Verification/FiniteGraph/fixtures/voteproof")
        #expect(try String(contentsOf: fixtures.appendingPathComponent("VoteProof.tla"), encoding: .utf8) == bundle.root.tla)
        #expect(try String(contentsOf: fixtures.appendingPathComponent("Consensus.tla"), encoding: .utf8) == consensus.tla)
        ...
        let safeAtRange = try #require(plusCal.range(of: "  SafeAt("))
        let chosenRange = try #require(plusCal.range(of: "  ChosenIn("))
