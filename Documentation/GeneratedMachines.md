...
The generated `verifySpec()` returns the number of explored states. It, `transitionMatrix()`, `verifyTransitions()`, and `verifyInvariants()` use a `100_000` state limit in the current macro generator. A limit failure is an error result. It does not establish a complete result outside that limit.
...
| Generated `verifySpec()` | Runs the generated bounded specification check and returns its explored-state count; it uses the current `100_000` state limit. | [MacroExpander+Generation.swift](../Sources/SwiftTLAPlugin/MacroExpander+Generation.swift) |
