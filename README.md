...
Every `#model` block runs the model checker at compile time. If invariants hold, the struct is generated. If they break, you get a compiler error with the counterexample trace.
...
`#model` produces working Swift. `apply()`, `transitions`, `availableActions`. Change the spec, the behavior changes.
...
```
`import SwiftTLA` for the DSL. `import SwiftTLAMacros` for `#model`.
