...
private enum AlgorithmSourceConstruct: Equatable {
    ...
    var invalidPlacement: InvalidAlgorithmComponent? {
        switch self {
        case .fairness: .genericFairness
        case .assume: .assumption
        default: nil
        }
    }
...
extension ParserSession {
    private func unsupportedAlgorithmSourceDiagnostic<Node: SyntaxProtocol>(
        ...
        return .init(
            ...
            message: "Algorithm declaration '\(sourceName)' is not supported.",
            source: sourceText,
            sourcePath: ["Algorithm", sourceName],
            sourceSpan: .init(
            ...
            actual: "unknown Algorithm declaration '\(sourceName)'",
    ...
    private func unsupportedAlgorithmSourceDiagnostic<Node: SyntaxProtocol>(
        ...
        return unsupportedAlgorithmSourceDiagnostic(named: sourceName, source: source)
    }

    private func recordAlgorithmSourceDiagnostic<Node: SyntaxProtocol>(
        named sourceName: String,
        source: Node
    ) {
        if algorithmSourceDiagnostic == nil {
            algorithmSourceDiagnostic = unsupportedAlgorithmSourceDiagnostic(
                named: sourceName,
                source: source
            )
        }
    ...
    private func recordInvalidAlgorithmPlacement<Node: SyntaxProtocol>(
        _ component: InvalidAlgorithmComponent,
        named sourceName: String,
        source: Node
    ) {
        guard algorithmSourceDiagnostic == nil else { return }
        let sourceText = source.description.trimmingCharacters(in: .whitespacesAndNewlines)
        algorithmSourceDiagnostic = .init(
            code: .invalidLanguagePlacement,
            message: "Algorithm declaration '\(sourceName)' is not valid in this position.",
            source: sourceText,
            sourcePath: ["Algorithm", sourceName],
            sourceSpan: .init(
                location: .utf8Offset(source.positionAfterSkippingLeadingTrivia.utf8Offset),
    ...
    private func parseAlgorithmComponent(
        ...
        case .fairness, .assume:
            guard let invalidPlacement = construct.invalidPlacement else { return nil }
            return .invalidPlacement(invalidPlacement)
        case .stateConstraint:
    ...
    private func parseProcedure(
        ...
        for item in closure.statements {
            ...
            guard let construct = AlgorithmSourceConstruct(call.calledExpression) else {
                ...
                recordAlgorithmSourceDiagnostic(named: sourceName, source: call)
                algorithmParseFailure = "Procedure '\(name)' accepts LocalVar declarations and Do or While blocks."
                return nil
            }
            if let invalidPlacement = construct.invalidPlacement {
                components.append(.invalidPlacement(invalidPlacement))
                continue
            }
    ...
    private func parseAlgorithmTemporal(
        ...
        case .leadsTo:
            guard arguments.count == 3,
                  let from = decodeTypedFacadeValue(arguments[1], scope: scope),
                  let to = decodeTypedFacadeValue(arguments[2], scope: scope)
            else { return nil }
    ...
    private func parseEach(
        ...
        for (index, statement) in closure.statements.enumerated() {
            ...
            guard let construct = AlgorithmSourceConstruct(componentCall.calledExpression) else {
                ...
                recordAlgorithmSourceDiagnostic(named: sourceName, source: componentCall)
                return nil
            }
            if let invalidPlacement = construct.invalidPlacement {
                components.append(.invalidPlacement(invalidPlacement))
    ...
    private func parseAlgorithmVariableDeclaration(
    ...
    ) -> (sourceName: String, component: AlgorithmComponentModel, shape: CompiledValueType?)? {
        ...
        do {
            valueType = try state.swiftTypeName.map(sourceTypeResolver.resolve) ?? inferredType
        } catch {
    ...
    private func parseAlgorithmStatements(
        ...
        for (index, statement) in statements.enumerated() {
            ...
            if let name = AlgorithmSourceConstruct.referenceName(in: call.calledExpression),
               macros[name] != nil {
                guard let expanded = parseMacroInvocation(
                    expression,
                    processParameter: processParameter,
                    macros: macros,
                    scope: scope
            ...
            if let invalidPlacement = construct.invalidPlacement {
                recordInvalidAlgorithmPlacement(
                    invalidPlacement,
                    named: AlgorithmSourceConstruct.referenceName(in: call.calledExpression) ?? "Algorithm declaration",
                    source: call
                )
                return nil
            }
    ...
    private func parseAlgorithmStatement(
        ...
        case .ifElse:
            ...
            let elseArguments = call.arguments.filter { $0.label?.text == "else" }
            guard call.additionalTrailingClosures.count + elseArguments.count <= 1,
                  call.additionalTrailingClosures.allSatisfy({ $0.label.text == "else" })
            else { return nil }
            let elseSyntax = call.additionalTrailingClosures.first.map { ExprSyntax($0.closure) }
                ?? elseArguments.first?.expression
            let otherwise: [AlgorithmStatementModel]
            if let elseSyntax {
                guard let closure = elseSyntax.as(ClosureExprSyntax.self),
                      let statements = parseAlgorithmStatements(
                          closure.statements,
                          processParameter: processParameter,
                          macros: macros,
        ...
        case .with:
            ...
            let selectedShapes = call.arguments.map {
                typedFacadeValueType($0.expression, scope: scope)?.selectedElement
            }
            let bindings = closureParameterNames(in: closure)
        ...
        case .letBinding:
            ...
            let replacement = generatedBinderName(line: UInt(call.positionAfterSkippingLeadingTrivia.utf8Offset), column: 0)
            let shape = typedFacadeValueType(valueSyntax, scope: scope)
            guard let body = parseAlgorithmStatements(
                closure.statements,
                processParameter: processParameter,
                macros: macros,
