...
struct NativeSwiftEmitter {
    ...
    init(model: MacroCompilation, sharedTypes: NativeTypeDeclarations? = nil) {
        ...
        let identifiers = GeneratedMachineAPI.generatedIdentifiers(
            properties.map { $0.declaration.name }, fallback: "property")
    ...
    /// Plan only functions reached by native emission, once per function identity.
    private mutating func functionPlan(for id: ResolvedFunctionID) -> NativeFunctionPlan {
        if let plan = functionPlans[id] { return plan }
        let plan = NativeFunctionPlan(function: id, functions: program.functions)
        functionPlans[id] = plan
        return plan
    }
    ...
    func swiftType(_ type: CompiledValueType) throws -> String {
        switch type {
        case .int: return "Int"
    ...
    func literal(_ value: CompiledValue, as type: CompiledValueType) throws -> String {
        ...
        if case .named(let name) = type {
            if let members = program.enums.cases[name],
               let item = members.first(where: { $0.value == value }) {
        ...
        case (.record(let value), .record(let fields)), (.record(let value), .nominalRecord(_, let fields)):
            ...
            let arguments = try fields.enumerated().map { index, field in
                ...
                return "\(fieldName(type, index: index, escaped: false)): \(try literal(value, as: field.type))"
            }
    ...
    func projected(_ value: String, from source: CompiledValueType, to destination: CompiledValueType?) throws -> String {
        ...
        let sourceFields: [CompiledValueType]?
        let destinationFields: [CompiledValueType]?
        ...
        if let sourceFields, let destinationFields {
            let arguments = try zip(sourceFields, destinationFields).enumerated().map { index, pair in
                "\(fieldName(destination, index: index, escaped: false)): \(try projected("source." + fieldName(source, index: index), from: pair.0, to: pair.1))"
            }.joined(separator: ", ")
        ...
        switch source {
        case .named(let name):
            guard let members = program.enums.cases[name] else { throw unsupported("enum declaration for \(name)") }
            cases = try members.map { item in
                "case .`\(item.name)`: return \(try literal(item.value, as: destination))"
            }.joined(separator: "\n")
    ...
    func ordering(_ type: CompiledValueType) throws -> String {
        let name = try swiftType(type)
        let body: String
        switch type {
    ...
    private mutating func emitFunction(
        ...
        """
        let body: String
        let functionPlan = functionPlan(for: id)
        if case .loop(let parameterOrder, let tailPlan) = functionPlan {
            ...
            let deferredInputs = try resolved.parameters.compactMap { parameter, type -> String? in
            ...
            for (parameter, type) in resolved.parameters where !eagerParameters.contains(parameter) {
    ...
    private mutating func recursiveBody(
        _ body: NativeFunctionPlan.Body, function: ResolvedFunctionID, depthGuard: String,
        state: String, substitutions: [BinderID: String], activeFunctions: Set<ResolvedFunctionID>, hasPendingReturns: Bool
    ) throws -> String {
        switch body {
        ...
        case .resume(let operand, let before, let body, let continuation):
            ...
            for id in before {
                let name = "_returnOperand\(ordinal(for: id))"
            ...
            let returnedOperand = "_returnedOperand\(ordinal(for: operand))"
        ...
        case .repeatCall(let arguments):
            let parameterOrder = functionPlan(for: function).parameterOrder
            let assignments = try zip(program[function].parameters.map(\.binder), arguments).map { parameter, argument in
                if !parameterOrder.contains(parameter) {
        ...
        case .call(let target, let arguments, let body):
            ...
            var nested = substitutions
            var bindings: [String] = []
            for ((parameter, type), argument) in zip(callee.parameters, arguments) {
    ...
    private mutating func expressionBody(
        ...
        case .value, .stateVariable, .boundValue, .controlLocation, .enabledAction,
             .assertView, .convert, .equal, .notEqual,
    ...
    private mutating func arithmeticExpression(
        ...
        while let (id, expanded) = pending.popLast() {
            ...
            if let value = expressionValues[id] {
                ...
                let rightFirst = node.operation.evaluatesRightOperandFirst
                if !expanded {
    ...
    private mutating func scalarExpression(
        ...
        let node = id
        let expression = node.operation
        func childType(_ index: Int) -> CompiledValueType { node.children[index].resultType }
    ...
    private mutating func collectionExpression(
        ...
        case .setLiteral:
            ...
            return "Set<\(try swiftType(element))>([\(try node.children.indices.map { try emit($0) }.joined(separator: ", "))])"
        case .tupleLiteral:
        ...
        case .cardinality: return "(\(try emit(0)).count)"
        case .integerRange: return "(try _NativeMachineOperations.integerRange(\(try emit(0)), \(try emit(1))))"
        case .setFilter(let binding):
            guard case .set(let element) = node.resultType else { throw unsupported("set filter") }
            return "Set<\(try swiftType(element))>(try \(try emit(0)).sorted(by: \(try ordering(element))).filter { (\(binder(binding)): \(try swiftType(element))) throws -> Bool in \(try emit(1)) })"
        case .setMap(let binding):
    ...
    private mutating func sequenceExpression(
        ...
        case .foldFunction(let parameters):
            guard parameters.count == 2 else { throw unsupported("fold arity") }
            var nested = substitutions
            nested[parameters[0]] = binder(parameters[0])
            nested[parameters[1]] = binder(parameters[1])
            ...
            return "(try \(elements).reversed().reduce(\(try emit(1))) { \(binder(parameters[1])), \(binder(parameters[0])) in \(body) })"
        case .sequenceSelect(let binding):
            let source = childType(0)
            let elements = try nativeSequenceElements(emit(0), source: source)
            return "(try \(elements).filter { (\(binder(binding))) throws -> Bool in \(try emit(1)) })"
    ...
    private mutating func functionExpression(
        ...
        case .functionLiteral(let binding):
            guard case .dictionary(let input, let result) = node.resultType else { throw unsupported("function literal") }
        ...
        case .functionApply:
            let argument = node.children[1].operation
            let result = node.resultType
            let source = childType(0)
            let access: String
            switch source {
            ...
            let evaluationOrder = node.operation.evaluatesRightOperandFirst ? Array(operands.reversed()) : operands
            return """
            (try { () throws -> \(try swiftType(result)) in
                \(evaluationOrder.joined(separator: "\n"))
                \(access)
            }())
            """
    ...
    private mutating func aggregateExpression(
        ...
        switch expression {
        case .except:
            let key = node.children[1].operation
            let originalType = childType(0)
            let update: String
            var retainsOriginal = true
            var evaluatesKey = true
            switch originalType {
            case .dictionary:
            ...
            case .record(let fields), .nominalRecord(_, let fields):
                ...
                func replacing(_ selected: Int) -> String {
                    let arguments = fields.indices.map { index in
                        ...
                        return "\(fieldName(originalType, index: index, escaped: false)): " + (index == selected ? "_replacementValue" : "_originalValue.\(name)")
                    }.joined(separator: ", ")
                    return "return \(nativeName)(\(arguments))"
                }
                if case .value(.string) = key {
                    update = selected.map(replacing) ?? "return _originalValue"
                    retainsOriginal = selected == nil || fields.count > 1
                    evaluatesKey = false
                } else {
            ...
            (try { () throws -> \(try swiftType(originalType)) in
                ...
                \(retainsOriginal ? "let _originalValue" : "_") = \(try emit(0))
                \(evaluatesKey ? "let _updatedKey = \(try emit(1))" : "")
                \(update)
            }())
            """
    ...
    private mutating func controlExpression(
        ...
        case .caseExpr(let hasOtherwise):
            let result = node.resultType
            var body = ""
            for index in 0..<(node.children.count / 2) {
                body += "if \(try emit(index * 2)) { return \(try emit(index * 2 + 1)) }\n"
            }
            body += hasOtherwise ? "return \(try emit(node.children.count - 1))" : "throw NativeMachineEvaluationError.noMatchingCase"
...
private func nativeSequenceElementType(_ source: CompiledValueType) throws -> CompiledValueType {
    switch source {
...
private func nativeSequenceElements(_ code: String, source: CompiledValueType) throws -> String {
