...
extension TLASpecVerifier {
    static func sourceTypes(
        ...
        let records = try schemas.reduce(into: [String: [NativeSourceRecordField]]()) { records, schema in
            ...
            guard let fieldsAlias = members.compactMap({ $0.decl.as(TypeAliasDeclSyntax.self) }).first(where: { $0.name.text == "Fields" }),
                  ...
                  let nameFunction = members.compactMap({ $0.decl.as(FunctionDeclSyntax.self) }).first(where: { $0.name.text == "fieldName" }),
            ...
            for statement in body.statements {
                if let declaration = statement.item.as(VariableDeclSyntax.self),
                   ...
                   binding.initializer?.value.tokens(viewMode: .sourceAccurate).map(\.text) == [parameterName, "as", "AnyKeyPath"],
                   reference == parameterName {
                    reference = name
                          ...
                          let property = properties.first(where: {
                              condition.tokens(viewMode: .sourceAccurate).map(\.text) == [reference, "==", "\\", fieldsName, ".", $0.name]
            ...
            guard let fieldList = bindings.first(where: {
                $0.pattern.as(IdentifierPatternSyntax.self)?.identifier.text == "fields"
            ...
            let declaredProperties = try fieldList.elements.map { element -> (selector: String, property: String) in
                guard let call = element.expression.as(FunctionCallExprSyntax.self),
                      let field = call.arguments.first?.expression.as(DeclReferenceExprSyntax.self)?.baseName.text,
                      let fieldBinding = bindings.first(where: {
                          $0.pattern.as(IdentifierPatternSyntax.self)?.identifier.text == field
                      }),
                      let fieldCall = fieldBinding.initializer?.value.as(FunctionCallExprSyntax.self),
                      fieldCall.calledExpression.as(DeclReferenceExprSyntax.self)?.baseName.text == "field",
                      fieldCall.arguments.count == 1,
                      let keyPath = fieldCall.arguments.first?.expression,
                      let property = properties.first(where: {
                          keyPath.tokens(viewMode: .sourceAccurate).map(\.text) == ["\\", fieldsName, ".", $0.name]
                      }) else { throw failure }
