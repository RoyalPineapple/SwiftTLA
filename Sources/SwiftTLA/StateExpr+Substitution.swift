extension StateExpr {
    ...
    private static func replace(in expr: StateExpr, using replacements: Replacements) -> StateExpr {
        ...
        switch expr {
        case .sourceIssue, .parameter: return expr
        ...
        case .nextState(let value): return .nextState(sub(value))
        case .stutteringStep(let value, let predicate): return .stutteringStep(sub(value), sub(predicate))
    ...
    private static func transform(
        ...
        func visit(_ expression: StateExpr) -> StateExpr {
            switch expression {
            case .sourceIssue, .value, .variable, .parameter, .processLocalFamily, .currentProcess, .programCounter, .procedureStack, .controlLocation, .enabledAction: return expression
            ...
            case .nextState(let value): return .nextState(visit(value))
            case .stutteringStep(let value, let predicate): return .stutteringStep(visit(value), visit(predicate))
