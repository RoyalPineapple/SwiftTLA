...
package struct CompiledTypeChecker: Sendable {
    ...
    private mutating func checkAuthoredAssignment(_ target: CompiledAuthoredPlusCalLValue,
        ...
        let read = try checkOperand(target.expression)
        return .init(target: target.resolvingRead(read), value: try checkOperand(value, expected: read.resultType))
    }
    ...
    private func literal(_ value: CompiledValue, expected: CompiledValueType = .unknown) throws -> CompiledValueType {
        ...
        case .tuple(let values):
            if values.isEmpty, case .dictionary = expected {
                result = expected
            } else if case .tuple(let hints) = expected, hints.count == values.count {
    ...
    private mutating func inferTupleLiteral(_ expressions: [CompiledExpression], expected: CompiledValueType) throws -> CheckedType {
        if expressions.isEmpty, case .dictionary = expected {
            return .init(type: expected, computationType: expected, children: [])
        }
        let result: CompiledValueType
