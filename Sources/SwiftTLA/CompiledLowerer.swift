struct CompiledLowerer {
    ...
    func lower(spec: TLASpec) throws -> CompiledModel {
        ...
        let actions: [CompiledAction] = try spec.actions.map(lower)
    ...
    private func lower(_ action: ActionExpr, at path: String) throws -> CompiledActionExpr {
        switch action {
        case .assign(_, let value): return try .assign(variable(at: "\(path).assign"), lower(value, at: "\(path).value"))
        ...
        case .and(let lhs, let rhs): return try .and(lower(lhs, at: "\(path).left"), lower(rhs, at: "\(path).right"))
        case .or(let lhs, let rhs): return try .or(lower(lhs, at: "\(path).left"), lower(rhs, at: "\(path).right"))
        }
    }
