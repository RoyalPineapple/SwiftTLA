...
public struct CollectionVar<Element: Identifiable & Sendable, Value: TLAValueType>: Sendable {
  ...
  public func update(_ member: CollectionMember<Element>, to value: some TypedExpression<Value>) -> ActionExpr {
    guard member.owner == name else {
      return .assign(.named(name), .sourceIssue(.collectionMember(collection: name, owner: member.owner)))
    }
    return .assign(.named(name), .except(.variable(name), member.binding, value.stateExpr))
  }

  public func allSatisfy<Predicate: TypedExpression<Bool>>(
    file: StaticString = #fileID, line: UInt = #line, column: UInt = #column,
    _ predicate: (Expr<Value>) -> Predicate
  ) -> Expr<Bool> {
    let member = generatedBinderName(file: file, line: line, column: column)
    let value = Expr<Value>(.functionApply(.variable(name), .variable(member)))
    return Expr(.forAll(memberDomain, member, predicate(value).stateExpr))
  }

  public func contains<Predicate: TypedExpression<Bool>>(
    file: StaticString = #fileID, line: UInt = #line, column: UInt = #column,
    where predicate: (Expr<Value>) -> Predicate
  ) -> Expr<Bool> {
    let member = generatedBinderName(file: file, line: line, column: column)
    let value = Expr<Value>(.functionApply(.variable(name), .variable(member)))
    return Expr(.exists(memberDomain, member, predicate(value).stateExpr))
  }
