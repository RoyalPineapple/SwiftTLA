/// Hand-written `TLAMachine` mirror of `TLASpec.bfsChecker(maxStates:)`.
///
/// Prefer `@TLAModel struct BFSChecker` (`SwiftTLAModels`) for the checked
/// generated machine. This type remains for lightweight runtime stepping
/// without pulling the macro target into core.
public struct CheckerController: Equatable, Hashable, Codable, Sendable, TLAMachine {
    ...
    public static let initial = CheckerController(
        phase: phaseExploring, processed: 0, queued: 1, limit: 100
    )
    ...
    public var transitions: [(action: Transition, target: Self)] {
        ...
        case Self.phaseExploring where canProcess:
            return [
                (.stepDiscover, CheckerController(
                    phase: Self.phaseExploring,
                    processed: processed + 1,
                    queued: queued + 1,
                    limit: limit
                )),
                (.stepNoNew, CheckerController(
                    phase: Self.phaseExploring,
                    processed: processed + 1,
                    queued: queued,
                    limit: limit
        ...
        case Self.phaseExploring where processed >= queued && queued > 0 && processed < limit:
            return [
                (.complete, CheckerController(
                    phase: Self.phaseComplete, processed: processed, queued: queued, limit: limit
                )),
                (.deadlock, CheckerController(
                    phase: Self.phaseDeadlocked, processed: processed, queued: queued, limit: limit
                )),
            ]
        ...
        case Self.phaseExploring where queued == 0:
            return [
                (.violate, CheckerController(
                    phase: Self.phaseViolated, processed: processed, queued: queued, limit: limit
                )),
            ]
        default:
            return []
    ...
    public var availableTransitions: [Transition] { transitions.map(\.action) }
    ...
    public var description: String {
        "phase=\(phase) p=\(processed) q=\(queued) L=\(limit)"
    }
}
