...
public enum ParityCatalog {
    ...
    // MARK: ewd840/SyncTD — N=3, abstract termination detection → 9 states
    ...
    private static func syncTDSpec() -> TLASpec {
        let N = 3
        let nodes = Array(0..<N)
        let boolOpts: [TLAValue] = [.bool(false), .bool(true)]
        var activeFuncs: [TLAValue] = []
    ...
    // MARK: ChangRoberts — N=3, leader election → 132 states
    ...
    private static func changRobertsSpec() -> TLASpec {
        let N = 3
        let nodes = Array(1...N)
