...
// Port: 1:1. N=3, Id={1,2,3}. CHOOSE expanded to per-value actions.

extension Example {
    static let changRobertsN3 = Example.Entry(
        ...
        notes: "N=3, Id=i. CHOOSE per-value expansion. No picked variable.",
        matchesUpstreamTLC: true
    )
}

private func changRobertsSpec() -> TLASpec {
    let N = 3
    let nodes = Array(1...N)
    ...
    return TLASpec("ChangRoberts") {
        ...
        for i in nodes {
            ...
            for picked in nodes {
                // n1_forward: lost, forwards any message
                ...
                // n1_skip: cand sees larger ID, ignores
