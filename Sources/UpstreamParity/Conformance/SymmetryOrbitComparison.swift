...
package struct SymmetryOrbitComparator: Sendable {
  ...
  package func compare(_ input: SymmetryOrbitComparisonInput) throws -> SymmetryOrbitComparisonResult {
    ...
    guard runs.allSatisfy(\.isPassEligible) else {
      return .difference([try SymmetryOrbitDifference(
        kind: .incompleteRun,
    ...
    guard rawComparison.isConformant else {
      return .difference(try rawComparison.differences.map { difference in
        try SymmetryOrbitDifference(
          kind: difference.category == .states ? .rawStateSet : .rawGraph,
