...
public enum PublicWorkflowDiagnosticCheckStatusV1: String, Codable, Sendable, Equatable {
  case matched
  case differed
  case unavailable
}

public struct PublicWorkflowDiagnosticCheckV1: Codable, Sendable {
  ...
  public let actualOutcome: PublicWorkflowExpectedOutcomeV1?
  ...
  public init(
    id: String,
    command: String,
    status: PublicWorkflowDiagnosticCheckStatusV1,
    expectedOutcome: PublicWorkflowExpectedOutcomeV1,
    actualOutcome: PublicWorkflowExpectedOutcomeV1?,
...
public struct PublicWorkflowDiagnosticReportV1: Codable, Sendable {
  public static let schema = "PublicWorkflowDiagnosticReportV1"
  public let schema: String
  public let runID: UUID
  public let authority: PublicWorkflowEvidenceAuthorityV1
  public let claimStatus: String
  public let checks: [PublicWorkflowDiagnosticCheckV1]
  public let finalExitClass: PublicWorkflowAdmissionExitClassV1

  public init(runID: UUID, authority: PublicWorkflowEvidenceAuthorityV1, checks: [PublicWorkflowDiagnosticCheckV1]) {
...
public struct PublicWorkflowConformanceRunnerV1: Sendable {
  ...
  private struct Register: Decodable {
    ...
    struct Entry: Decodable {
      let id: String
      let manifest: CoreEvidenceReferenceV1
      let expectedOutcome: PublicWorkflowExpectedOutcomeV1
    }
  }
  ...
  @discardableResult
  public func run(_ options: Options) throws -> (report: PublicWorkflowDiagnosticReportV1, reportURL: URL) {
    ...
    let authority: PublicWorkflowEvidenceAuthorityV1 = hostedExecutionRequested(options.hostedCI) ? .candidate : .diagnostic
    let checks: [PublicWorkflowDiagnosticCheckV1]
    do {
      ...
      guard register.schema == Register.schema else {
        throw PublicWorkflowGovernanceErrorV1.invalidSchema(register.schema)
      }
    ...
    let report = PublicWorkflowDiagnosticReportV1(runID: runID, authority: authority, checks: checks)
  ...
  private func execute(
    ...
    var checks = [PublicWorkflowDiagnosticCheckV1]()
  ...
  private func platformCheck(root: URL, output: URL, gateRunID: UUID, hostedCI: Bool) throws -> PublicWorkflowDiagnosticCheckV1 {
    ...
    do {
      ...
      guard !platformStatuses.isEmpty else {
        throw PublicWorkflowGovernanceErrorV1.invalidField(record: "platform-matrix", field: "retained platform results")
      }
      let actual: PublicWorkflowExpectedOutcomeV1? = process.terminationStatus == 0 ? .exact : platformStatuses.contains("failed") ? .difference : nil
      let status: PublicWorkflowDiagnosticCheckStatusV1
      switch actual {
  ...
  private func fixtureCheck(root: URL, output: URL, gateRunID: UUID) throws -> PublicWorkflowDiagnosticCheckV1 {
    try FileManager.default.createDirectory(at: output, withIntermediateDirectories: true)
    let fixtures = [
    ...
    for (id, directory, kind, scheme, shouldBuild) in fixtures {
      ...
      do {
        ...
        try JSONSerialization.data(withJSONObject: ["schema": "PublicWorkflowAnnotationFixtureResultV1", "gateRunID": gateRunID.uuidString.lowercased(), "id": id, "expected": shouldBuild ? "succeeded" : "failed", "actualExit": process.terminationStatus, "matched": matched], options: [.sortedKeys]).write(to: result)
      } catch {
        unavailable = true
        try JSONSerialization.data(withJSONObject: ["schema": "PublicWorkflowAnnotationFixtureResultV1", "gateRunID": gateRunID.uuidString.lowercased(), "id": id, "status": "unavailable", "diagnostic": String(describing: error)], options: [.sortedKeys]).write(to: result)
      }
    }
    ...
    for reference in retained where reference.path.hasSuffix("/result.json") {
      ...
      guard value?["gateRunID"] as? String == gateRunID.uuidString.lowercased() else {
        throw PublicWorkflowGovernanceErrorV1.invalidField(record: reference.path, field: "aggregate annotation correlation")
      }
    }
    let status: PublicWorkflowDiagnosticCheckStatusV1 = unavailable ? .unavailable : differed ? .differed : .matched
    let actual: PublicWorkflowExpectedOutcomeV1? = status == .matched ? .exact : status == .differed ? .difference : nil
    return PublicWorkflowDiagnosticCheckV1(id: "annotation-fixtures", command: "xcodebuild annotation fixtures", status: status, expectedOutcome: .exact, actualOutcome: actual,
  ...
  private func parserCheck(_ entry: Register.Entry, root: URL, output: URL, gateRunID: UUID) throws -> PublicWorkflowDiagnosticCheckV1 {
  ...
  private func generatedCheck(_ id: String, expected: PublicWorkflowExpectedOutcomeV1, manifest: CoreEvidenceReferenceV1,
                              root: URL, output: URL, gateRunID: UUID) throws -> PublicWorkflowDiagnosticCheckV1 {
  ...
  private func check(id: String, command: String, expected: PublicWorkflowExpectedOutcomeV1,
                     actual: PublicWorkflowExpectedOutcomeV1, references: [CoreEvidenceReferenceV1], root: URL, output: URL) throws -> PublicWorkflowDiagnosticCheckV1 {
    let comparison = try reference(for: output.appendingPathComponent("comparison.json"), beneath: root)
    return PublicWorkflowDiagnosticCheckV1(id: id, command: command,
  ...
  private func correlation(caseID: String, gateRunID: UUID) throws -> PublicWorkflowCaseRunCorrelationV1 {
    try PublicWorkflowCaseRunCorrelationV1(caseID: caseID, gateRunID: gateRunID, fixtureRunID: UUID(), comparisonRunID: UUID())
  }

  private func verified(_ reference: CoreEvidenceReferenceV1, beneath root: URL) throws -> Data {
    let url = root.appendingPathComponent(reference.path).resolvingSymlinksInPath().standardizedFileURL
    guard url.path.hasPrefix(root.path + "/") else {
      throw PublicWorkflowGovernanceErrorV1.invalidField(record: reference.path, field: "path escape")
    }
    guard FileManager.default.fileExists(atPath: url.path) else {
      throw PublicWorkflowGovernanceErrorV1.invalidField(record: reference.path, field: "missing evidence")
    }
  ...
  private func reference(for url: URL, beneath root: URL) throws -> CoreEvidenceReferenceV1 {
    let resolved = url.resolvingSymlinksInPath().standardizedFileURL
    let prefix = root.path + "/"
    guard resolved.path.hasPrefix(prefix) else { throw PublicWorkflowGovernanceErrorV1.invalidField(record: resolved.path, field: "evidence path") }
    return try CoreEvidenceReferenceV1(path: String(resolved.path.dropFirst(prefix.count)), sha256: SHA256V1.hex(try Data(contentsOf: resolved)))
  }

  private func artifactReferences(in directory: URL, beneath root: URL) throws -> [CoreEvidenceReferenceV1] {
