...
public enum PublicWorkflowDivergenceClassification: String, Codable, Sendable {
  case swiftTLADefect
...
public enum PublicWorkflowDivergenceDisposition: String, Codable, Sendable {
  case open
  case resolved
  case unsupported
...
public struct PublicWorkflowDivergenceComparison: Equatable, Codable, Sendable {
  ...
  public func validate() throws {
    ...
    guard outcome == .difference ? normalizedDifferenceFingerprint?.isEmpty == false : normalizedDifferenceFingerprint == nil else {
      throw PublicWorkflowGovernanceError.invalidField(record: "comparison", field: "normalizedDifferenceFingerprint")
    }
  }
...
public struct PublicWorkflowDivergenceRecord: Equatable, Codable, Sendable {
  ...
  public let classification: PublicWorkflowDivergenceClassification
  public let disposition: PublicWorkflowDivergenceDisposition
  public let normalizedDifferenceFingerprint: String
  public let latestComparison: PublicWorkflowDivergenceComparison

  public init(id: String, caseID: String, semanticCitations: [String], reproducer: CoreFiniteBounds, originalEvidence: CoreEvidenceReference, permanentRegressionCaseID: String, classification: PublicWorkflowDivergenceClassification, disposition: PublicWorkflowDivergenceDisposition, normalizedDifferenceFingerprint: String, latestComparison: PublicWorkflowDivergenceComparison) throws {
    self.id = id
    self.caseID = caseID
  ...
  public func validate() throws {
    ...
    guard !id.isEmpty, !caseID.isEmpty, !permanentRegressionCaseID.isEmpty, !normalizedDifferenceFingerprint.isEmpty,
      ...
      throw PublicWorkflowGovernanceError.invalidField(record: id, field: "required evidence")
    }
    guard disposition != .resolved || latestComparison.outcome == .exact else {
      throw PublicWorkflowGovernanceError.invalidField(record: id, field: "latest comparison")
    }
    guard latestComparison.outcome != .difference || latestComparison.normalizedDifferenceFingerprint == normalizedDifferenceFingerprint else {
      throw PublicWorkflowGovernanceError.invalidField(record: id, field: "fingerprint drift")
    }
  }
...
public struct PublicWorkflowDivergenceLedger: Equatable, Codable, Sendable {
  ...
  public init(schema: String, records: [PublicWorkflowDivergenceRecord]) throws {
    ...
    for record in records {
      try record.validate()
      guard ids.insert(record.id).inserted else { throw PublicWorkflowGovernanceError.duplicateID(kind: "divergence", id: record.id) }
    }
    self.schema = schema
  ...
  public func validate(caseIDs: Set<String>) throws {
    for record in records {
      guard caseIDs.contains(record.caseID), caseIDs.contains(record.permanentRegressionCaseID) else {
        throw PublicWorkflowGovernanceError.unknownCaseID(caseIDs.contains(record.caseID) ? record.permanentRegressionCaseID : record.caseID)
      }
    }
  }
