            ...
            if current == 92 {
                ...
                case 117:
                    guard index + 4 <= bytes.count,
                          let scalar = UInt32(String(decoding: bytes[index..<index + 4], as: UTF8.self), radix: 16).flatMap(UnicodeScalar.init) else { throw error("invalid unicode escape") }
...
private func validateProvenance(_ value: JSONValue?) {
    ...
    guard string(provenance, "tlcTag") == "v1.8.0",
          string(provenance, "tlcCommit") == "30cc3601321c3fc02e044d0ecb5c58d8921e18df",
          string(provenance, "tlcJarSha256") == "e22f8ffb4bacdea0a871f444dd94fe5fb0d8013b3388ae39e82e26f852c735d5",
          ...
          let arguments = array(provenance, "arguments"), arguments.compactMap({ if case let .string(value) = $0 { value } else { nil } }) == expectedArguments,
...
for (index, record) in records.enumerated() {
    ...
    case "transition":
        ...
        guard string(record, "callback") == "writeState.action", let action = object(record, "action"), let flags = object(record, "stateFlags") else {
    ...
    case "footer":
        ...
        guard index == records.count - 1, string(record, "callback") == "writer.close", string(record, "status") == "closed",
              number(record, "lastBodySeq") == index - 1, let digest = string(record, "bodySha256"), case let .object(footerCounts)? = record["counts"] else {
...
guard footerSeen, counts == ["header": 1, "initial": 2, "transition": 9],
      ...
      observedStates == Set([StateRef(x: "0", level: 1), StateRef(x: "1", level: 1), StateRef(x: "2", level: 2), StateRef(x: "3", level: 3), StateRef(x: "3", level: 4), StateRef(x: "4", level: 4), StateRef(x: "5", level: 5)]),
